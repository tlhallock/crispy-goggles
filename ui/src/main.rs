...
impl fmt::Display for RenderError {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		match self {
			RenderError::MissingTasks => {
				write!(f, "No tasks available for rendering")
			}
			RenderError::MissingStartTime => {
				write!(f, "Missing start time for rendering")
			}
...
impl From<DrawingRect> for common::model::Rec {
	fn from(r: DrawingRect) -> Self {
		(
			Point {
				x: r.start_x as Coord,
				y: r.start_y as Coord,
			},
			Point {
				x: r.current_x as Coord,
				y: r.current_y as Coord,
			},
		)
			.into()
	}
...
fn apply_event(state: &mut UiState, ev: &Event) {
	...
	match kind {
		...
		common::grpc::event::Kind::Hide(h) => {
			...
			state.last_unit_pos.remove(&h.id);
		}
		_ => {}
	}
}
...
fn App() -> impl IntoView {
	...
	// Update bounds display when trigger changes
	{
		...
		Effect::new(move |_| {
			...
			let bottom_right =
				state.zoom.pixel_to_map((width, height), &canvas);
	...
	let on_wheel = move |ev: web_sys::WheelEvent| {
		...
		state.zoom.pixels_per_meter =
			state.zoom.pixels_per_meter.clamp(1.0, 1000.0);
